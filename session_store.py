...
def init_db():
    ...
    # Migration: add task_id / worker_id columns to event_log if they don't exist (backward compat)
    for col, col_type in [("task_id", "TEXT DEFAULT NULL"), ("worker_id", "TEXT DEFAULT NULL")]:
        try:
            cur.execute(f"ALTER TABLE event_log ADD COLUMN {col} {col_type}")
...
def create_task(title: str, description: str, goal: str, parent_id: str = None) -> str:
    ...
    _log(conn, "task", task_id, "created", {"title": title}, task_id=task_id)
    conn.commit()
    conn.close()
    return task_id
...
def update_task_status(task_id: str, status: str, result: str = None, error: str = None):
    ...
    _log(conn, "task", task_id, f"status_{status}", {"result": result, "error": error}, task_id=task_id)
    conn.commit()
    conn.close()
...
def create_worker(task_id: str, worker_id: str, worker_type: str, goal: str, context: dict = None, max_retries: int = 3, timeout: int = 7200, priority: int = 0) -> str:
    ...
    _log(conn, "worker", worker_id, "created", {"task_id": task_id, "worker_type": worker_type, "timeout": timeout, "priority": priority}, task_id=task_id, worker_id=worker_id)
    conn.commit()
    conn.close()
    return worker_id
...
def update_worker_status(worker_id: str, status: str, result: str = None, error: str = None, session_id: str = None):
    ...
    _log(conn, "worker", worker_id, f"status_{status}", {"result": result, "error": error}, task_id=None, worker_id=worker_id)
    conn.commit()
    conn.close()
...
def increment_worker_retry(worker_id: str) -> int:
    ...
    _log(conn, "worker", worker_id, "retry", {"retry_count": retry_count}, task_id=None, worker_id=worker_id)
    conn.commit()
    conn.close()
    return retry_count
...
def _log(conn, entity_type: str, entity_id: str, event: str, detail: dict = None, task_id: str = None, worker_id: str = None):
    """写事件日志"""
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO event_log (entity_type, entity_id, event, detail, task_id, worker_id) VALUES (?, ?, ?, ?, ?, ?)""",
        (entity_type, entity_id, event, json.dumps(detail or {}), task_id, worker_id)
    )


def harness_event_log(conn, event_type: str, task_id: str, worker_id: str, details: dict = None):
    """记录 harness 层事件（task_id/worker_id 由调用方传入，timestamp 由 DEFAULT 自动生成）"""
    _log(conn, "harness", event_type, event_type, details, task_id, worker_id)


def worker_pool_event_log(conn, event_type: str, task_id: str, worker_id: str, details: dict = None):
    """记录 worker_pool 层事件（task_id/worker_id 由调用方传入，timestamp 由 DEFAULT 自动生成）"""
    _log(conn, "worker_pool", event_type, event_type, details, task_id, worker_id)
